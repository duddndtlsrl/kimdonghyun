...
void Save(Player* P_List[])
{
	int Num;
	scanf("%d", &Num);
	FILE* f = foepn("SaveFile%d.txt", Num, "w");
...
void Load(Player* P_List[])
{
	int Num;
	scanf("%d", &Num);
	FILE* f = foepn("SaveFile%d.txt", Num, "r");
	fsanf(f, "%s%d%d%d%d%d%d%d%d", P_List[0]->Name, &P_List[0]->Num, &P_List[0]->Att, &P_List[0]->CurrHp, &P_List[0]->MaxHp, &P_List[0]->CurrExp, &P_List[0]->MaxExp, &P_List[0]->GetExp, &P_List[0]->Lev);
	fsanf(f, "%d", ECount);
	for (int i = 1; i <= EMax; i++)
		fscanf(f, "%s%d%d%d%d%d%d%d%d", P_List[i]->Name, &P_List[i]->Num, &P_List[i]->Att, &P_List[i]->CurrHp, &P_List[i]->MaxHp, &P_List[i]->CurrExp, &P_List[i]->MaxExp, &P_List[i]->GetExp, &P_List[i]->Lev);
	fclose(f);
